...
class ComplainFeedBackPanelModel{
  ...
  Future<ComplainFeedBackPanelModel?> getUserData(int ID) async {
    ...
    List<Map<String, dynamic>>? maps = await db!.rawQuery("SELECT iduser,username,image,location from user where iduser = ${ID}");
    if(maps.length==1)
      {
        ComplainFeedBackPanelModel user = ComplainFeedBackPanelModel.fromMap(maps[0]);
        // print(user.username);
        return user;
      }
    return null;
